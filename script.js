...
// Stagger cards within the same grid/row so they cascade in smoothly instead of popping together.
setTimeout(() => {
  const groups = [
  ...
  groups.forEach(([parentSel, itemSel]) => {
    document.querySelectorAll(`${parentSel} ${itemSel}`).forEach((el, i) => {
